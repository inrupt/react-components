  ...
  subscribe(...urls) {
    for (let url of urls) {
      ...
      if (!(url in subscribers)) {
        ...
        if (url !== ALL) trackResource(url);else fetchedUrls.forEach(fetchedUrl => trackResource(fetchedUrl));
...
function trackResource(url, webSocketOptions = {}) {
  ...
  let webSocket = webSockets[host]; // If no socket exists, create a new one
